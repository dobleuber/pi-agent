...
function getModeBorderColor(theme: any, mode: string): (text: string) => string {
	...
	// Default: derive from the last observed thinking level instead of consulting
	// the live extension context during editor render. The old extension instance
	// can become stale during reload/session teardown while the TUI still renders
	// one more frame.
	const thinkingLevel =
		(mode === CUSTOM_MODE_NAME ? customOverlay?.thinkingLevel : spec?.thinkingLevel) ?? lastObservedThinkingLevel;
...
let lastObservedThinkingLevel: ThinkingLevel = "off";
...
async function applyMode(pi: ExtensionAPI, ctx: ExtensionContext, mode: string): Promise<void> {
	...
	if (mode === CUSTOM_MODE_NAME) {
		...
		lastObservedThinkingLevel = customOverlay.thinkingLevel ?? lastObservedThinkingLevel;
	...
	try {
		...
		if (spec.thinkingLevel) {
			...
			lastObservedThinkingLevel = spec.thinkingLevel;
		}
	} finally {
...
function setEditor(pi: ExtensionAPI, ctx: ExtensionContext, history: PromptEntry[]) {
	...
	ctx.ui.setEditorComponent((tui, theme, keybindings) => {
		...
		const bashModeBorderColor = uiTheme.getBashModeBorderColor();
		const borderColor = (text: string) => {
			const isBashMode = editor.getText().trimStart().startsWith("!");
			if (isBashMode) {
				return bashModeBorderColor(text);
			}
			return getModeBorderColor(uiTheme, runtime.currentMode)(text);
		};
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		lastObservedThinkingLevel = pi.getThinkingLevel();
		await ensureRuntime(pi, ctx);
		customOverlay = null;
