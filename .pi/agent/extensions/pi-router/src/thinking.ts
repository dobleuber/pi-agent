...
export function resolveWorkProfile(input: ResolveWorkProfileInput): WorkProfileDecision {
	...
	if (managed) {
		if (override.level && THINKING_RANK[override.level] < THINKING_RANK.high) selectedModel = MANAGED_PROFILE_EVIDENCE.luna.model;
		else if (override.level === "max") selectedModel = MANAGED_PROFILE_EVIDENCE.solMax.model;
