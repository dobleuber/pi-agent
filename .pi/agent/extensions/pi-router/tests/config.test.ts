...
describe("router configuration", () => {
	...
	it("defines a remote GPT-5.4 Mini router profile through OpenAI Codex subscription auth", () => {
		assert.equal(DEFAULT_ROUTER_CONFIG.routerModels.remote.provider, "openai-codex");
		assert.equal(DEFAULT_ROUTER_CONFIG.routerModels.remote.model, "gpt-5.4-mini");
		assert.equal(DEFAULT_ROUTER_CONFIG.routerModels.remote.baseUrl, "https://chatgpt.com/backend-api");
		assert.equal(resolveRouterModel({ ...DEFAULT_ROUTER_CONFIG, localMode: "off" }).model, "gpt-5.4-mini");
	});
	...
	it("builds status summary with remote router model when local mode is off", () => {
		...
		assert.equal(summary, "router:on local:off routerModel:openai-codex/gpt-5.4-mini workModel:unknown");
	});

	it("does not show a separate target when the effective router model matches the configured model", () => {
		const summary = routerStatusSummary({
			config: { ...DEFAULT_ROUTER_CONFIG, state: "on", localMode: "off" },
