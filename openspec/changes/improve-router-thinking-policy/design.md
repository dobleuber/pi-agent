...
Model routing activates only when the current work model already belongs to the `openai-codex/gpt-5.6-luna|terra|sol` family. A manually selected ZAI, Anthropic, or other external model is preserved; in that case the policy adjusts only thinking effort and records `modelRouting: "preserved-external"`.
