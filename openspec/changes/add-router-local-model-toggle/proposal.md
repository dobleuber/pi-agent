...
- Add `/router local off` to select the remote `openrouter/openai/gpt-5.4-nano` router model and stop the configured local llama.cpp server.
...
- Remote routing dependency: OpenRouter-compatible model `openrouter/openai/gpt-5.4-nano` at `https://openrouter.ai/api/v1`.
