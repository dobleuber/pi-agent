...
- Add `/router local off` to select the remote `openai-codex/gpt-5.4-nano` router model and stop the configured local llama.cpp server.
...
- Remote routing dependency: OpenAI Codex subscription model `openai-codex/gpt-5.4-nano` at `https://chatgpt.com/backend-api`, resolved via Pi's `ctx.modelRegistry`/OAuth auth rather than OpenRouter or direct OpenAI API-key calls. If the installed Codex catalog does not expose Nano yet, the router may use its configured Codex fallback model.
