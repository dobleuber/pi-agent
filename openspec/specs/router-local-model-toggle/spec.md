...
Define Pi router local-mode behavior, including independent router enablement, local llama.cpp lifecycle management, remote OpenAI Codex subscription routing, and persistent shared state.
...
The router SHALL support `/router local off` to select the remote `openai-codex/gpt-5.4-mini` router model and stop the local llama.cpp server for the configured local router model.

#### Scenario: Switching to remote mode
- **WHEN** the user runs `/router local off`
- **THEN** the router MUST select `openai-codex/gpt-5.4-mini` as the active router model
...
- **THEN** prompt routing and final-answer translation MUST use `openai-codex/gpt-5.4-mini` at `https://chatgpt.com/backend-api`

#### Scenario: Codex Mini is unavailable
- **WHEN** routing is enabled, local mode is off, and `ctx.modelRegistry` does not find `openai-codex/gpt-5.4-mini`
- **THEN** prompt routing and final-answer translation MUST preserve passthrough-with-warning behavior without using OpenRouter or direct OpenAI API-key calls
