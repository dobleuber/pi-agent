---
name: openscad
description: "Create and render OpenSCAD 3D models on Windows. Generate preview images from multiple angles, extract customizable parameters, validate syntax, and export STL files for 3D printing platforms like MakerWorld."
---

# OpenSCAD Skill (Windows)
...
OpenSCAD must be installed on Windows. Install using one of:
```powershell
...
This skill provides several PowerShell tools in the `tools/` directory:

### Preview Generation
```powershell
# Generate a single preview image
.\tools\preview.ps1 model.scad output.png -Camera "x,y,z,tx,ty,tz,dist" -Size "800x600"

# Generate multi-angle preview (front, back, left, right, top, iso)
.\tools\multi-preview.ps1 model.scad .\output_dir\
```

### STL Export
```powershell
# Export to STL for 3D printing
.\tools\export-stl.ps1 model.scad output.stl
.\tools\export-stl.ps1 model.scad output.stl -Define "param=value","param2=value2"
```

### Parameter Extraction
```powershell
# Extract customizable parameters from an OpenSCAD file
.\tools\extract-params.ps1 model.scad
.\tools\extract-params.ps1 model.scad -Json
```

### Validation
```powershell
# Check for syntax errors and warnings
.\tools\validate.ps1 model.scad
```

### Render with JSON Parameters
```powershell
# Render using parameters from a JSON file
.\tools\render-with-params.ps1 model.scad params.json output.stl
.\tools\render-with-params.ps1 model.scad params.json preview.png
```
...
1. **Generate multi-angle previews** using `multi-preview.ps1`
...
```powershell
.\tools\multi-preview.ps1 model.scad .\previews\
```
...
.\tools\export-stl.ps1 model.scad output.stl -Define "width=60","height=40"
```
...
1. STL file(s) exported via `export-stl.ps1`
...
# 2. Validate syntax
.\tools\validate.ps1 box.scad

# 3. Generate multi-angle previews
.\tools\multi-preview.ps1 box.scad .\previews\
...
.\tools\extract-params.ps1 box.scad

# 6. Export STL with default parameters
.\tools\export-stl.ps1 box.scad box.stl

# 7. Export STL with custom parameters
.\tools\export-stl.ps1 box.scad box_large.stl -Define "width=80","height=60"
```
